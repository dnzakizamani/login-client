...
export default function MoodboardDetail() {
  ...
  const fetchProject = async () => {
    try {
      const res = await api.get(`/api/moodboards/${id}`)
  ...
  const fetchImages = async () => {
    try {
      const res = await api.get(`/api/moodboards/${id}/images`)
  ...
  const handleFileUpload = async (files) => {
    ...
    try {
      ...
      await api.post(`/api/moodboards/${id}/images`, formData, {
  ...
  const handleDrop = async (e) => {
    ...
    try {
      await api.put(`/api/moodboards/${id}/images/${draggedImage.id}`, {
  ...
  const handleDeleteImage = async (imageId) => {
    ...
    try {
      await api.delete(`/api/moodboards/${id}/images/${imageId}`)
  ...
  const handleImageResize = async (imageId, newWidth, newHeight) => {
    try {
      await api.put(`/api/moodboards/${id}/images/${imageId}`, {
        width: newWidth,
        height: newHeight
      })
          ...
          >
            {images.length === 0 ? (
              ...
              images.map((image) => (
                ...
                >
                  <img
                    src={`${api.defaults.baseURL}/uploads/${image.filename}`}
        ...
        {selectedImage && (
          <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50" onClick={() => setSelectedImage(null)}>
            <div className="relative max-w-4xl max-h-screen p-4">
              <img
                src={`${api.defaults.baseURL}/uploads/${selectedImage.filename}`}
