...
export default function Images() {
  ...
  const fetchImages = async () => {
    try {
      const res = await api.get('/api/image-files')
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      await api.post('/api/image-files/upload', uploadData, {
  ...
  const handleWatermark = async (e) => {
    ...
    try {
      await api.post(`/api/image-files/${selectedImage.id}/watermark`, watermarkData)
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await api.delete(`/api/image-files/${id}`)
            ...
            {images.map((image) => (
              <div key={image.id} className="border border-gray-200 dark:border-gray-700 rounded-lg p-4 hover:shadow-md transition-shadow">
                ...
                <div className="flex gap-2 flex-wrap">
                  ...
                  <button
                    onClick={() => window.open(`${api.defaults.baseURL}/api/image-files/${image.id}/download`, '_blank')}
                  ...
                  {image.watermark_text && (
                    <button
                      onClick={() => window.open(`${api.defaults.baseURL}/api/image-files/${image.id}/download/watermarked`, '_blank')}
