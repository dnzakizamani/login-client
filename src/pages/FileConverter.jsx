...
export default function FileConverter() {
  ...
  const fetchSupportedConversions = async () => {
    try {
      const res = await api.get('/api/file-conversions/supported')
  ...
  const fetchConversionHistory = async () => {
    try {
      const res = await api.get('/api/file-conversions/history')
  ...
  const handleConvert = async () => {
    ...
    try {
      const res = await api.post('/api/file-conversions/convert', formData, {
        headers: {
  ...
  const pollConversionStatus = (conversionId) => {
    const poll = async () => {
      try {
        const res = await api.get(`/api/file-conversions/status/${conversionId}`)
  ...
  const downloadFile = async (conversionId, filename) => {
    try {
      const response = await api.get(`/api/file-conversions/download/${conversionId}`, {
        responseType: 'blob'
      })
  ...
  const deleteConversion = async (conversionId) => {
    ...
    try {
      await api.delete(`/api/file-conversions/${conversionId}`)
