...
export default function PDFReader() {
  ...
  const fetchPDF = async () => {
    try {
      const res = await api.get(`/api/pdf-files/${id}`)
      const pdfData = res.data.pdf
      
      // Also fetch the PDF file content with authentication for the viewer
      const pdfRes = await api.get(`/api/pdf-files/${id}/download`, {
        responseType: 'blob'
      })
      const pdfBlob = new Blob([pdfRes.data], { type: 'application/pdf' })
      const pdfUrl = URL.createObjectURL(pdfBlob)
      
      // Set the pdf state with both metadata and URL
      setPdf({
        ...pdfData,
        url: pdfUrl
      })
  ...
  const loadReadingProgress = async () => {
    try {
      const res = await api.get(`/api/pdf-files/${id}/progress`)
  ...
  const saveReadingProgress = async () => {
    try {
      await api.post(`/api/pdf-files/${id}/progress`, {
  ...
  const translateText = async (text) => {
    ...
    setTranslating(true)
    try {
      const response = await api.post(`/api/pdf-files/${id}/translate`, {
        text, from: 'en', to: 'id'
      })
  ...
  const downloadPDF = async () => {
    try {
      const response = await api.get(`/api/pdf-files/${id}/download`, {
        responseType: 'blob',
      })
      const url = window.URL.createObjectURL(new Blob([response.data]))
      const link = document.createElement('a')
      link.href = url
      link.setAttribute('download', pdf?.original_filename || 'document.pdf')
    ...
    <div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded shadow p-4">
        <div ref={pdfContainerRef} className="flex justify-center overflow-auto" onMouseUp={handleTextSelection}>
          <Document file={pdf?.url || `${api.defaults.baseURL}/api/pdf-files/${id}/download`}
